 ...
 *
 * Limitations:
 * - Conversion timing is instantaneous. A conversion runs synchronously when an
 *   input, the calibration or the configuration changes; the per-channel
 *   conversion times (VSH_CT/VBUS_CT), the averaging count (AVG) and continuous
...
struct INA230State {
    ...
    bool alert_latched; /* latched alert pending Mask/Enable read */

    char *description;
};

static bool ina230_limit_alert(const INA230State *s)
{
...
static void ina230_refresh_alert(INA230State *s)
{
    ...
    bool aff = latch ? s->alert_latched : ina230_limit_alert(s);
    bool cnvr;
    bool active;
    int level;
...
static void ina230_sample_alert(INA230State *s)
{
    if (FIELD_EX16(s->mask_enable, INA230_MASK_ENABLE, LEN) &&
        ina230_limit_alert(s)) {
                           ...
                           void *opaque, Error **errp)
{
    ...
    if (value < 0 || value > INA230_BUS_MAX_UV) {
        error_setg(errp,
                   ...
                   " out of range (0..%d uV)",
                   __func__, s->description, value, INA230_BUS_MAX_UV);
        return;
    }
