 ...
 *   input, the calibration, the ADC range or the ADC configuration changes; the
 *   ADC conversion times (VBUSCT/VSHCT/VTCT), the averaging count (AVG) and the
...
/*
 * The datasheet is self-inconsistent on the TEMP_LIMIT reset value: the
 * register description lists 0x7FFF, yet TOL occupies bits [15:4] and declares
 * the low nibble [3:0] reserved/read-zero. The model resolves the conflict in
 * favour of the reserved-bit rule and seeds 0x7FF0 so [3:0] read back as zero.
 * The choice is immaterial to behaviour anyway: writes mask with
 * INA238_TEMP_LIMIT_MASK and the alert comparison shifts both operands >>4.
 */
 ...
 * It is the SAME in both ranges: at ADCRANGE=1 the shunt register is four
 * times larger for a given voltage (1.25 uV LSB), and the datasheet
 * (SS 7.1.2, eq. 1) requires the host to program SHUNT_CAL four times larger
 * to match, so the register-domain divisor stays 4096. (Using 1024 here would
 * report a quarter of the true current whenever ADCRANGE=1.)
 ...
 * register is read; in transparent mode it tracks _cond_ directly.
 */
#define INA238_UPD_FLAG(_diag_, _latch_, _field_, _cond_)                     \
    do {                                                                      \
        uint16_t _old_val_ = FIELD_EX16(_diag_, INA238_DIAG_ALRT, _field_);   \
        uint16_t _new_val_ = (uint16_t)((_cond_) ||                           \
                                        ((_latch_) && _old_val_));            \
        (_diag_) = FIELD_DP16(_diag_, INA238_DIAG_ALRT, _field_, _new_val_);  \
    } while (0)
...
static void ina238_update_alert(INA238State *s)
{
    ...
    INA238_UPD_FLAG(diag, latch, SHNTOL, shunt > (int16_t)s->sovl);
    INA238_UPD_FLAG(diag, latch, SHNTUL, shunt < (int16_t)s->suvl);
    INA238_UPD_FLAG(diag, latch, BUSOL,
                    s->bus_reg > (s->bovl & INA238_BUS_LIMIT_MASK));
    INA238_UPD_FLAG(diag, latch, BUSUL,
                    s->bus_reg < (s->buvl & INA238_BUS_LIMIT_MASK));
    INA238_UPD_FLAG(diag, latch, TMPOL, temp > temp_lim);
    INA238_UPD_FLAG(diag, latch, POL,
...
static void ina238_reset_regs(INA238State *s)
{
    ...
    /*
     * The measurement registers power up at zero (datasheet Tables 6-8 through
     * 6-12) and stay zero until the next conversion. The injected physical
     * inputs are retained (they represent the voltages still present on the
     * pins), but no conversion is performed here, so a software reset does not
     * resurrect a previous measurement. CNVRF stays clear until the guest
...
static uint32_t ina238_read_reg(INA238State *s, uint8_t reg)
{
    ...
    case INA238_REG_DIAG_ALRT:
         ...
         * only in latch mode (transparent flags track the live condition).
         */
...
static void ina238_write_reg(INA238State *s, uint8_t reg, uint16_t val)
{
    ...
    case INA238_REG_DIAG_ALRT: {
         ...
         * conversion-ready flag (Table 6-13: "cleared by reading or writing to
         * this register ... when ALATCH = 1"). In transparent mode the flags
         * track the live condition and are preserved. The pin is only refreshed
         * (not re-sampled), so a cleared flag re-asserts on the next conversion
         * rather than immediately.
         */
        if (FIELD_EX16(s->diag_alrt, INA238_DIAG_ALRT, ALATCH)) {
            status &= ~(R_INA238_DIAG_ALRT_CNVRF_MASK | INA238_DIAG_LATCH_MASK);
        }
                             ...
                             void *opaque, Error **errp)
{
    ...
    if (value < INA238_SHUNT_MIN_NV || value > INA238_SHUNT_MAX_NV) {
        error_setg(errp,
                   "%s: %s: shunt-voltage %" PRId64
                   " out of range (%d..%d nV)",
                   __func__, s->description, value,
                           ...
                           void *opaque, Error **errp)
{
    ...
    if (value < 0 || value > INA238_BUS_MAX_UV) {
        error_setg(errp,
                   "%s: %s: bus-voltage %" PRId64
                   " out of range (0..%d uV)",
                   __func__, s->description, value, INA238_BUS_MAX_UV);
        return;
    }
