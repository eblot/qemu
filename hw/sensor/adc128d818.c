...
#define BUSY_STATUS_NOT_READY   (1u << 1u)
...
/* One-Shot Register (0x09) bitfields */
#define ONE_SHOT_TRIGGER        0x01u
...
/* Conversion times (for reference, not used for actual delays) */
#define ADC128D818_VOLTAGE_CONV_US      12000u
#define ADC128D818_TEMP_CONV_US         3600u
...
static bool adc128d818_is_temp_channel(const ADC128D818State *s, unsigned ch)
{
    ...
    /* Mode 1: all 8 channels are voltage inputs, no temperature */
    return adc128d818_get_mode(s) != 1u;
}

static bool adc128d818_is_reserved_channel(const ADC128D818State *s, unsigned ch)
{
...
/*
 * Pseudo-differential pair definitions per the ADC128D818 datasheet.
 * Some pairs reverse the numerical pin order (IN3+/IN2-, IN7+/IN6-).
 */
...
static void adc128d818_check_limits(ADC128D818State *s)
{
    ...
    for (unsigned ch = 0u; ch < ADC128D818_NUM_CHANNELS; ch++) {
        ...
        msb = (uint8_t)(s->channel[ch] >> 8u);
        high_lim = s->regs[REG_LIMIT_BASE + ch * 2u];
        low_lim = s->regs[REG_LIMIT_BASE + ch * 2u + 1u];
...
static void adc128d818_convert(ADC128D818State *s)
{
    ...
    /* Clear NOT_READY after first successful conversion */
...
static void adc128d818_write_reg(ADC128D818State *s, uint8_t reg, uint8_t val)
{
    ...
    case REG_CH_DISABLE:
        ...
        break;
    case REG_ONE_SHOT:
        if ((val & ONE_SHOT_TRIGGER) && !(s->regs[REG_CONFIG] & CONFIG_START)) {
            adc128d818_convert(s);
        }
        break;
    case REG_DEEP_SHUTDOWN:
    ...
    case REG_ADV_CONFIG: {
        ...
        old_mode = s->regs[REG_ADV_CONFIG] & ADV_CONFIG_MODE_MASK;
        s->regs[REG_ADV_CONFIG] = val & ADV_CONFIG_WR_MASK;
        if ((val & ADV_CONFIG_MODE_MASK) != old_mode) {
            memset(s->channel, 0, sizeof(s->channel));
            s->regs[REG_INT_STATUS] = 0x00u;
            adc128d818_update_irq(s);
        }
                               ...
                               void *opaque, Error **errp)
{
    ...
    if (rc != 1 || ch_num < 0 || ch_num >= (int)ADC128D818_NUM_CHANNELS) {
        error_setg(errp, "%s: invalid channel '%s'", __func__, name);
        return;
    }
                               ...
                               void *opaque, Error **errp)
{
    ...
    if (value < INT16_MIN || value > INT16_MAX) {
        error_setg(errp, "%s: value %" PRId64 " out of range for '%s'",
                   __func__, value, name);
        return;
    }
    ...
    Object *obj, Visitor *v, const char *name, void *opaque, Error **errp)
{
    ...
    if (value < INT32_MIN || value > INT32_MAX) {
        error_setg(errp, "%s: value %" PRId64 " out of range", __func__,
                   value);
        return;
    }
    ...
    Object *obj, Visitor *v, const char *name, void *opaque, Error **errp)
{
    ...
    if (value < 0 || value > ADC128D818_MAX_VDD_MV) {
        error_setg(errp,
                   "%s: ext-vref-mv %" PRId64 " out of range (0..%u mV)",
                   __func__, value, ADC128D818_MAX_VDD_MV);
        return;
    }
